...
class _Coco(data.Dataset):
    ...
    def __init__(self, config=None, split=None, purpose=None, preload=False):
        ...
        self.single_mode = hasattr(
            config, "single_mode") and config.single_mode
    ...
    def _prepare_test(self, index, img, label):
        ...
        if RENDER_DATA:
            render(label, mode="label", name=(
                "test_data_label_pre_%d" % index))
        ...
        if RENDER_DATA:
            ...
            render(label, mode="label", name=(
                "test_data_label_post_%d" % index))
            render(mask, mode="mask", name=("test_data_mask_%d" % index))

        # dataloader must return tensors (conversion forced in their IIC anyway)
...
class _Coco164k(_Coco):
    ...
    def _set_files(self):
        ...
        if self.split in ["train2017", "val2017"]:
            ...
            file_list = [f.split("/")[-1].replace(".jpg", "")
                         for f in file_list]
            self.files = file_list
        else:
            raise ValueError("Invalid split name: {}".format(self.split))
    ...
    def _load_data(self, image_id):
        # Set paths
        image_path = osp.join(self.root, "images",
                              self.split, image_id + ".jpg")
...
class _Coco164kCuratedFew(_Coco):
    ...
    def _set_files(self):
        # Create data list by parsing the "images" folder
        if self.split in ["train2017", "val2017"]:
            file_list = osp.join(self.root, "curated",
                                 self.split, self.name + ".txt")
            file_list = tuple(open(file_list, "r"))
            file_list = [id_.rstrip() for id_ in file_list]
    ...
    def _make_fine_to_few_dict(self):
        ...
        for c in xrange(182):
            ...
            if orig_coarse_ind in self.label_orig_coarse_inds:
                new_few_ind = self.label_orig_coarse_inds.index(
                    orig_coarse_ind)
