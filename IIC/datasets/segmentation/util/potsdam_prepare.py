...
def main():
    ...
    unlabelled_train = open(os.path.join(
        OUT_DIR, "unlabelled_train.txt"), "w+")
...
def split_and_save_imgs(img, next_index, names, cut, rescale, dir):
    ...
    for i_h in xrange(num_blocks):
        for i_w in xrange(num_blocks):
            ...
            sio.savemat(os.path.join(dir, "%s.mat" %
                                     str(name)), {"img": img_curr})
            offset += 1
...
def split_and_save_gts(gt, next_index, names, cut, rescale, dir):
    ...
    for i_h in xrange(num_blocks):
        for i_w in xrange(num_blocks):
            start_h = i_h * cut
            start_w = i_w * cut
            flat_gt_curr = flat_gt[start_h:(
                start_h + cut), start_w:(start_w + cut)]
            name = names[next_index + offset]
            sio.savemat(os.path.join(dir, "%s.mat" %
                                     str(name)), {"gt": flat_gt_curr})
            offset += 1
